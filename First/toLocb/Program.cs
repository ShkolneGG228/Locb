...
namespace toLocb
{
    class Program
    {
        ...
        {
            Console.WriteLine("1 - Удаление по индексу\n2 - Удаление по значению");
            string input="",key;
            key = Console.ReadLine();
            ...
            do
            {
                switch (key)
                {
                    case "1":
                        {
                            input = Console.ReadLine();
                            if (IsNumberContains(input) && Convert.ToInt32(input)< mas.Count) { mas.RemoveAt(Convert.ToInt32(input)); }
                            else if(input != "STOP") { Console.WriteLine("Ошибка(не число или введеное число больше длины массива)"); }
                        }break;
